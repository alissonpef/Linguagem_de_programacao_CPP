    ...
    ui(new Ui::fm_gestaoEstoque)
{
    ...
    ui->tw_ge_produtos->setColumnCount(2); // Teremos 2 tabelas
    ui->tabWidget->setCurrentIndex(0); // Iniciar na aba certa
}
...
void fm_gestaoEstoque::on_btn_novoproduto_clicked() // Botao de novo produto
{
    ...
    ui->txt_codigoproduto->setFocus(); // Posiciona o cursor no codigo do produto
}


void fm_gestaoEstoque::on_btn_gravarnovoproduto_clicked() // botao de Salvar
{
        // Armazena as variaveis
        QString aux; // Variavel auxiliar
...
void fm_gestaoEstoque::on_tabWidget_currentChanged(int index) // Atualiza a aba de gestão de estoque
{
        if(index == 1) // Segunda aba da gestão
        {
            ...
            if(query.exec())
            {
                while(query.next())
                {
                    ...
                    ui->tw_ge_produtos->setRowHeight(contlinhas, 20); // Tamanho das linhas
                    contlinhas++;
                }
                ui->tw_ge_produtos->setColumnWidth(0, 150); // Largura das colunas
                ui->tw_ge_produtos->setColumnWidth(1, 230); // Largura das colunas
                QStringList cabecalhos = {"Código", "Produto"}; // Arquivo de cabeçalho
                ui->tw_ge_produtos->setHorizontalHeaderLabels(cabecalhos);
                ui->tw_ge_produtos->setStyleSheet("QTableView {selection-background-color: red}"); // Cor da linha selecionada
                ui->tw_ge_produtos->setEditTriggers(QAbstractItemView::NoEditTriggers); // Nao deixa editar
                ui->tw_ge_produtos->setSelectionBehavior(QAbstractItemView::SelectRows); // Selecionar a linha inteira
                ui->tw_ge_produtos->verticalHeader()->setVisible(false); // Cabeçalho vertical
            }
            else
            {
...
void fm_gestaoEstoque::on_tw_ge_produtos_itemSelectionChanged() // Seleção com teclado ou mouse
{
    ...
    if(query.exec())
    {
            // Mostra os valores do item selecionado
            query.first();
...
void fm_gestaoEstoque::on_btn_ge_gravar_clicked() // Botão gestão de estoque / salvar
{
    if(ui->txt_ge_codigoproduto->text()=="") // Se estiver vazio
    {
...
void fm_gestaoEstoque::on_btn_ge_excluir_clicked() // Botão de excluir
{
    if(ui->txt_ge_codigoproduto->text()=="") // Primeiro verifica se tem algo selecionado
    {
    ...
    else // Excluir
    {
        ...
        if(opc == QMessageBox::Yes)
        {
            ...
            if(query.exec())
            {
                ui->tw_ge_produtos->removeRow(linha); // Remove a linha
                QMessageBox::information(this,"Deletado","Produto foi removido com sucesso");
            }
            else
            {
...
void fm_gestaoEstoque::on_btn_ge_filtrar_clicked() // Botão de pesquisa
{
    QString busca;
    funcoes_globais::removerLinhas(ui->tw_ge_produtos);
    ...
    {
        QMessageBox::warning(this,"ERRO","Erro ao filtrar os produtos.");
    }
